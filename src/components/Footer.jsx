      ...
      <div className="mx-auto max-w-[1400px] px-5 md:px-8 py-20 md:py-24">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 lg:gap-10">
          <div className="lg:col-span-1">
            <Link to="/" className="font-heading text-xl font-bold text-white tracking-tight">
              Wanderlust
              <span className="inline-block w-1.5 h-1.5 rounded-full bg-primary-500 ml-0.5 mb-3" />
            </Link>
          ...
          <div>
            <h4 className="text-[11px] font-semibold tracking-[0.15em] uppercase text-stone-500 mb-5">Quick Links</h4>
            <div className="grid grid-cols-2 gap-x-6 gap-y-2.5">
              {footerNav.map((n) => (
                <Link key={n.to} to={n.to} className="text-sm text-stone-400 hover:text-white transition-colors">
                  {n.label}
                </Link>
              ))}
            </div>
          ...
          <div>
            <h4 className="text-[11px] font-semibold tracking-[0.15em] uppercase text-stone-500 mb-5">Popular Destinations</h4>
            <div className="flex flex-col gap-2.5">
              {popularDestinations.map((d) => (
                <Link key={d.to} to={d.to} className="text-sm text-stone-400 hover:text-white transition-colors">
                  {d.label}
                </Link>
              ))}
            </div>
          ...
          <div>
            <h4 className="text-[11px] font-semibold tracking-[0.15em] uppercase text-stone-500 mb-5">Contact</h4>
            <div className="flex flex-col gap-3 text-sm text-stone-400">
              <div className="flex items-start gap-3">
                <svg className="w-4 h-4 text-stone-500 mt-0.5 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M15 10.5a3 3 0 11-6 0 3 3 0 016 0z" />
                  <path strokeLinecap="round" strokeLinejoin="round" d="M19.5 10.5c0 7.142-7.5 11.25-7.5 11.25S4.5 17.642 4.5 10.5a7.5 7.5 0 1115 0z" />
                </svg>
                <span>Kochi, Kerala, India</span>
              </div>
              <a href="tel:+919100527275" className="flex items-center gap-3 hover:text-white transition-colors">
                <svg className="w-4 h-4 text-stone-500 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M2.25 6.75c0 8.284 6.716 15 15 15h2.25a2.25 2.25 0 002.25-2.25v-1.372c0-.516-.351-.966-.852-1.091l-4.423-1.106c-.44-.11-.902.055-1.173.417l-.97 1.293c-.282.376-.769.542-1.21.38a12.035 12.035 0 01-7.143-7.143c-.162-.441.004-.928.38-1.21l1.293-.97c.363-.271.527-.734.417-1.173L6.963 3.102a1.125 1.125 0 00-1.091-.852H4.5A2.25 2.25 0 002.25 4.5v2.25z" />
                </svg>
                <span>+91 91005 27275</span>
              </a>
              <a href="mailto:info@wanderlusttravels.in" className="flex items-center gap-3 hover:text-white transition-colors">
                <svg className="w-4 h-4 text-stone-500 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
        ...
        <div className="border-t border-white/5 mt-16 pt-8 flex flex-col md:flex-row justify-between items-center gap-4">
          ...
          <div className="flex gap-6">
            <Link to="/faq" className="text-[12px] text-stone-500 hover:text-stone-300 transition-colors">Privacy Policy</Link>
            <Link to="/faq" className="text-[12px] text-stone-500 hover:text-stone-300 transition-colors">Terms of Service</Link>
          </div>
        </div>
      </div>
    </footer>
