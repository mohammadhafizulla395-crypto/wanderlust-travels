...
export default function Tours() {
  ...
  return (
    <div>
      <section className="relative bg-gradient-to-br from-neutral-900 via-neutral-800 to-secondary-700 text-white py-16 md:py-20 overflow-hidden">
        ...
        <div className="max-w-[1320px] mx-auto px-6 lg:px-8">
            ...
            <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
              ...
              <div className="relative w-full sm:w-72 flex-shrink-0">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-4 py-2.5 border border-neutral-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-secondary-500/20 focus:border-secondary-500 transition-all"
                />
              </div>
            </div>
          </div>
          ...
          {filtered.length === 0 ? (
            <div className="text-center py-20">
              ...
              <button
                onClick={resetFilters}
                className="inline-flex items-center gap-2 bg-secondary-500 hover:bg-secondary-600 text-white font-medium px-6 py-3 rounded-lg transition-colors text-sm"
              >
                View all tours
              </button>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
              {filtered.map((tour, i) => (
                ...
                >
                  <Link
                    to={`/tours/${tour.slug}`}
                    className="group block bg-white rounded-xl overflow-hidden hover:shadow-xl transition-all duration-500 h-full"
                  >
                    <div className="relative aspect-[4/3] overflow-hidden">
                      ...
                      <div className="absolute top-3 left-3 flex gap-2">
                        <span className="bg-white/90 backdrop-blur-sm text-neutral-700 text-xs font-medium px-3 py-1.5 rounded-lg">
                          {tour.category}
                        </span>
                      </div>
                      <div className="absolute top-3 right-3">
                        <span className="bg-secondary-500 text-white text-xs font-bold px-2.5 py-1.5 rounded-lg">
                    ...
                    <div className="p-5 flex flex-col flex-1">
                      ...
                      <h3 className="font-heading text-lg font-semibold mb-2 group-hover:text-secondary-500 transition-colors">
                        {tour.name}
                      </h3>
                      ...
                      <div className="flex gap-2">
                        <span className="flex-1 text-center bg-secondary-500 text-white font-medium text-sm py-2.5 rounded-lg group-hover:bg-secondary-600 transition-colors">
                          View Details
                        </span>
                        <Link
                          to={`/booking?tour=${encodeURIComponent(tour.name)}`}
                          onClick={(e) => e.stopPropagation()}
                          className="flex-1 text-center border border-secondary-200 text-secondary-600 font-medium text-sm py-2.5 rounded-lg hover:bg-secondary-50 transition-colors"
                        >
                          Enquire
                        </Link>
                      </div>
